    ...
    namespace helper {
        ...
        std::string extract_pubkey_from_cert(const std::string &certstr, const std::string &pw = "") {
...
#else
            std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(BIO_new_mem_buf(certstr.data(), certstr.size()),
                                                                  BIO_free_all);
#endif
            ...
            if (!PEM_write_bio_PUBKEY(keybio.get(), key.get()))
                throw rsa_exception("Error writing public key data in PEM format");
            char *ptr = nullptr;
        ...
        load_public_key_from_string(const std::string &key, const std::string &password = "") {
            ...
            std::shared_ptr<EVP_PKEY> pkey(
                    PEM_read_bio_PUBKEY(pubkey_bio.get(), nullptr, nullptr, (void *) password.c_str()), EVP_PKEY_free);
            if (!pkey)
        ...
        inline
        std::shared_ptr<EVP_PKEY>
        load_private_key_from_string(const std::string &key, const std::string &password = "") {
    ...
    namespace algorithm {
        ...
        struct hmacsha {
            ...
            std::string sign(const std::string &data) const {
                ...
                if (HMAC(md(), secret.data(), secret.size(), (const unsigned char *) data.data(), data.size(),
                         (unsigned char *) res.data(), &len) == nullptr)
        ...
        struct rsa {
            ...
            rsa(const std::string &public_key, const std::string &private_key, const std::string &public_key_password,
                const std::string &private_key_password, const EVP_MD *(*md)(), const std::string &name)
                    : md(md), alg_name(name) {
            ...
            void verify(const std::string &data, const std::string &signature) const {
                ...
                auto res = EVP_VerifyFinal(ctx.get(), (const unsigned char *) signature.data(), signature.size(),
                                           pkey.get());
                if (res != 1)
                    throw signature_verification_exception("evp verify final failed: " + std::to_string(res) + " " +
        ...
        struct ecdsa {
            ...
            std::string sign(const std::string &data) const {
                ...
                std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)>
                        sig(ECDSA_do_sign((const unsigned char *) hash.data(), hash.size(), pkey.get()),
                            ECDSA_SIG_free);
                if (!sig)
                    throw signature_generation_exception();
#ifdef OPENSSL10
        ...
        struct pss {
            ...
            std::string sign(const std::string &data) const {
                ...
                if (!RSA_padding_add_PKCS1_PSS_mgf1(key.get(), (unsigned char *) padded.data(),
                                                    (const unsigned char *) hash.data(), md(), md(), -1))
                    throw signature_generation_exception(
                            "failed to create signature: RSA_padding_add_PKCS1_PSS_mgf1 failed");

                std::string res(size, 0x00);
                if (RSA_private_encrypt(size, (const unsigned char *) padded.data(), (unsigned char *) res.data(),
            ...
            void verify(const std::string &data, const std::string &signature) const {
                ...
                if (!RSA_verify_PKCS1_PSS_mgf1(key.get(), (const unsigned char *) hash.data(), md(), md(),
    ...
    public:
        ...
        void verify(const decoded_jwt &jwt) const {
            ...
            if (jwt.has_expires_at()) {
                auto leeway =
                        claims.count("exp") == 1 ? std::chrono::system_clock::to_time_t(claims.at("exp").as_date())
                                                 : default_leeway;
            ...
            if (jwt.has_issued_at()) {
                auto leeway =
                        claims.count("iat") == 1 ? std::chrono::system_clock::to_time_t(claims.at("iat").as_date())
                                                 : default_leeway;
            ...
            if (jwt.has_not_before()) {
                auto leeway =
                        claims.count("nbf") == 1 ? std::chrono::system_clock::to_time_t(claims.at("nbf").as_date())
                                                 : default_leeway;
